...
\end{itemize}
Die Gleichgewichtspunkte entlang des nicht-trivialen L"osungszweiges sind somit asymptotisch stabil, wo sich jener f"ur $x>0$ nach rechts wendet ($(\lambda^*)'>0$) und instabil, wo sich jener f"ur $x<0$ nach links wendet ($(\lambda^*)'(x)<0$).
\qed
\end{beweis}
\begin{figure}[htpb]
		\centering
...
\mu'(0) &= 1 > 0 (\lambda_C = 0)
\end{align*}
mit $\lambda_C = 0)$. Aber au"ser diesen trivialen L"osungen gibt es keine weiteren Gleichgewichtspunkte. Dazu setze man $v(\lambda,x,y) = 0$:
\begin{itemize}
\item
