...
Als Voraussetzung der Grobman-Hartman-Theorie gen"ugt es, wenn die Voraussetzungen lokal nahe der betrachteten Gleichgewichtspunkte erfüllt sind.
...
\begin{figure}[htpb]
		\centering
		\includegraphics[width=0.5\textwidth]{img/periodische_orbits/disk_kont.pdf}
...
$$\Sigma_{x_p} := \left \{ \left. x \in \RR \right| \langle x - x_p, n \rangle \right \}$$ 
schneidet den Orbit $\Gamma_{x_p}$ \emph{transversal}. Sei $V\subseteq \RR^n$ eine hinreichend kleine Umgebung von $x_p$. Die \emph{erste R"uckkehrzeit} $\tau$ ist definiert als
$$\tau(x) := \min \left \{ \left. t> 0 \right| \phi(t, x) \in \Sigma_{x_p} \right \}$$
\end{definition}
\begin{description}
\item[Bemerkung] 
Die erste R"uckkehrzeit gibt die Zeit an, die ben"otigt wird um, ausgehend vom Punkt $x \in \Sigma_{x_p}\cap V$, die transversale Menge $\Sigma_{x_p}$ wieder zu schneiden. Das hei"st es gilt $\phi(\tau(x), x) \in \Sigma_{x_p}$. Es gilt nach Definition $\tau(x_p) = T$.
\end{description}
\begin{figure}[htpb]
		\centering
...
Sei $v \in C^k(\RR^n,\RR^n)$ ein Vektorfeld mit $k \in \mathbb{N}$. Dann ist $\tau \in C^k(V, \RR)$, wobei $V\subseteq \RR^n$ hinreichend klein.
\end{lemma}
\begin{beweis}
...
\begin{figure}[htpb]
		\centering
		\includegraphics[width=0.3\textwidth]{img/periodische_orbits/orbiale_stabilitaet.pdf}
		\includegraphics[width=0.3\textwidth]{img/periodische_orbits/orbitale_asymp_stabilitaet.pdf}
...
Sei $\Gamma_{x_p}$ ein periodischer Orbit von $(X, \phi)$ und $P_{\Sigma_{x_p}}$ eine zugeh"orige Poincar\'{e} Abbildung. Dann sind "aquivalent
\begin{enumerate}
\item $x_p$ ist ein (asymptotisch) stabiler Fixpunkt von $P_{\Sigma_{x_p}}^N, \ N\in\mathbb{N}$ im Sinne von Lyapunov
\item $\Gamma_{x_p}$ ist ein orbital (asymptotisch) stabiler periodischer Orbit von $(X, \phi)$.
\end{enumerate}
\end{satz}
...
\begin{figure}[htpb]
		\centering
		\includegraphics[width=0.3\textwidth]{img/periodische_orbits/beispiel_asymp_stabilitaet.pdf}
