...
Betrachte $(X,\Phi)$, so wie $(X,\Psi)$ mit $\Psi(x)=\frac x {1+\mu}$\\
$\Rightarrow\ $ jeder Orbit von $(X,\Psi)$ n"ahert sich asymptotisch $x=0$ an, und damit keine sensitive Abh"angigkeit von Anfangsdaten im Gegensatz zu $(X,\Phi)$. Aber beide Systeme sind topologisch konjugiert bzgl. des Hom"oomorphismus $h:X\rightarrow X, x\mapsto \frac 1 x$
\[\Phi(h(x))=(1+\mu)h(x)=\frac{1+\mu} x, h(\Psi(x))=\frac 1 {\Psi(x)}=\frac 1 {\frac x {1+\mu}} =\frac{1+\mu} x\]
\[\Rightarrow\ h(\Psi(x))=\Phi(h(x))\]
\end{beispiel}

\begin{definition}[Devaney 1989]
Ein diskretes dynamisches System $(X,\Phi)$ wie oben mit $X$ unendliche Menge heißt (im deterministischen Sinn) chaotisch, falls es transitiv ist und falls die periodischen Orbits (Punkte) dicht in $X$ liegen.
\end{definition}

\begin{bemerkung}
Sei $X$ endlich, $(X,\Phi)$ transitiv $\Rightarrow\ X=\Gamma_{x_0}$ besteht aus einem einzigen periodischen Orbit, d.h. $(X,\Phi)$ ist nicht chaotisch. Denn sei $\varepsilon >0$ hinreichend klein, so dass $x_j\notin\operatorname{B}_{\varepsilon}(x_k)$ f"ur $k\neq j$.\\
Transitivit"at: Es gibt ein $N\in\mathbb{N}$, so dass
\[\Phi^N(\operatorname{B}_{\varepsilon}(x_0))\cap \operatorname{B}_{\varepsilon}(x_j)\neq\emptyset\ \forall j\neq 0\]
...
\[d(\Phi^{l+j}(q),\Phi^{l+j}(y))\geq\underbrace{\operatorname{dist}(x,\Gamma_p)}_{\geq 4\Lambda}-\underbrace{d(x,\underbrace{\Phi^{l+j}(q)}_{=q})}_{<\varepsilon<\Lambda}-\underbrace{\operatorname{dist}(\Phi^{l+j}(y),\Gamma_p)}_{<\Lambda}\geq 2\Lambda\]
Dreiecksungleichung (nach oben):
\[d(\Phi^{l+j}(x),\Phi^{l+j}(q))\geq\Lambda\text{ oder } d(\Phi^{l+j}(x),\Phi^{l+j}(y))\geq\Lambda\]
Sonst
\[d(\Phi^{l+j}(q),\Phi^{l+j}(y))\leq\underbrace{d(\Phi^{l+j}(q),\Phi^{l+j}(x))}_{<\Lambda}+\underbrace{d(\Phi^{l+j}(x),\Phi^{l+j}(y))}_{<\Lambda}<2\Lambda\]
Da $x,\varepsilon$ beliebig, folgt sensitive Abh"angigkeit von Anfangsdaten mit Sensibilit"atskonstanten $\Lambda$. \qed
\end{beweis}

\begin{beispiel}
\begin{enumerate}
\item Bernoulli-Shift-Dynamik $(\Sigma_2,\Phi_\sigma)$ ist chaotisch, denn dieses System besitzt wie gezeigt einen dichten Orbit ($\Rightarrow$ Transitivit"at) und die periodischen Orbits liegen dicht in $\Sigma_2$

\item Hier betrachten wir Systeme, die von nicht notwendigerweise invertierbaren Abbildungen $\Phi:X\rightarrow X$ erzeugt werden, "Vorw"artsdynamik"', z.B. die Zeltabbildung als erzeugende Abbildung in $I=\mathbb{R}, \mu\in\mathbb{R}, \mu>0: T_{\mu}:= \frac{\mu} 2 (1-|2x-1|)$
...
Transitivit"at: F"ur alle $x\in[0,1]$ und $\varepsilon>0$ gibt es $n,j\in\matnbb{N}$, so dass $I_{n,j}\in\operatorname{B_{\varepsilon}(x)=(x-\varepsilon,x+\varepsilon)$. Sei $V\subset X=[0,1]$ eine beliebige offene Teilmenge
...
$\Rightarrow$ das "Semisystem" $([0,1],T_2)$ ist chaotisch. Die Invertierbarkeit des erzeugenden Hom"oomorphismus wurde zuvor im Zusammenhang von Chaos nirgends benutzt.

\item Sei $\mu\in\mathbb{R}, 0<\mu\leq 4, L_{\mu}:[0,1]\rightarrow[0,1], x\mapsto\mu x(1-x)$ polynomial, $C^{\infty}$-glatt\\
...
Der eingef"uhrte Chaosbegriff bleibt unter topologischer Konjugation erfüllt.
\end{beispiel}
